...
```{py:function} Open(key)
Set the IP2WHOIS API key.

:param string key: (Required) IP2WHOIS API key.
```
...
```{py:staticmethod} toPunycode(domain)
...
```{py:staticmethod} toNormalText(domain)
Get Normal Text.
...
```{py:staticmethod} toDomainName(url)
...
```{py:staticmethod} toDomainExtension(str)
